...
set of hand-written Kubernetes YAML files. That `pi_cluster`-side change is
...
session secret). That Secret is created by `pi_cluster`'s SOPS flow, same as
...
helm install kabom charts/kabom \
  --set env.s3Endpoint=minio.pi10.szatanik.dev:9000 \
...
(defaults to `kabom.szatanik.dev`), resource limits, `persistence.storageClass`
— has a working default.

**`persistence.storageClass` must stay `local-path`, never `nas-smb`** — see
