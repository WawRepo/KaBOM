...
Wires kabom.db's run_ingest into the app:
...
GET /healthz: no auth, no DB dependency, always ok
if the process is up.

The browser UI sits on top of the same data: three Jinja2-rendered
...
# Freshness thresholds for the banner. Chosen against the default
...
async def lifespan(app: FastAPI):
    ...
    # misconfigured auth mode must stop the app from ever serving
...
def get_db_connection() -> Iterator[sqlite3.Connection]:
    ...
    compromise, at a few dozen SBOMs.
...
def search(
    ...
    correct and fast at a few dozen SBOMs. Every result row carries its
...
def _search_rows(conn: sqlite3.Connection, q: str, version: str | None = None) -> list[dict]:
    """Shared by GET /api/search and the "/" search page — one
...
def _sboms_rows(conn: sqlite3.Connection) -> list[dict]:
    """Shared by GET /api/sboms and the "/sboms" inventory page."""
    rows = conn.execute(
...
def _sbom_detail_row(conn: sqlite3.Connection, sbom_id: int) -> dict | None:
    ...
    None means no sbom with that id."""
    row = conn.execute(
...
def status(conn: sqlite3.Connection = Depends(get_db_connection)) -> dict:
    ...
    stuck at 40 days while every other one refreshes nightly is exactly the case
...
def _status_dict(conn: sqlite3.Connection) -> dict:
    ...
    — the banner reads exactly what this endpoint reports, never
...
# --- The UI — three screens and the freshness banner ------------------------
#
