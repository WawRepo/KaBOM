...
background timer — those are app-wiring concerns for kabom.main (the search
...
def get_connection(db_path: str) -> sqlite3.Connection:
    ...
    `check_same_thread=False`: kabom.main opens one connection per HTTP request
...
def infer_kind(source_key: str) -> str:
    ...
    kabom.ingest.ParsedSBOM carries no explicit kind field, but schema.sql's
    `sbom.kind` column requires one of 'image' or 'host'. The producing job
    lays files out under a leading "hosts/" or "images/" key
